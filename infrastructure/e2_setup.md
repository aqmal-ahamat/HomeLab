...
### Phase 3: Docker Compose with Caddy Reverse Proxy
...
Create `e2_docker-compose.yml` with both n8n and Caddy services:
```yaml
version: '3.8'
...
services:
  n8n:
    ...
    environment:
      - N8N_HOST=0.0.0.0
      - N8N_PORT=5678
      - N8N_PROTOCOL=https  # HTTPS behind reverse proxy
      - N8N_EXECUTIONS_DATA_SAVE_ON_ERROR=all
      - N8N_EXECUTIONS_DATA_SAVE_ON_SUCCESS=all
      - N8N_METRICS=true
      - N8N_SECURE_COOKIE=true  # Enabled for HTTPS
    networks:
      - n8n-internal
  ...
  caddy:
    ...
    ports:
      ...
      - "443:443"
    volumes:
      - ./e2_caddyfile:/etc/caddy/Caddyfile:ro
      - e2_caddy_data:/data
    networks:
      - n8n-internal
    depends_on:
...
#### 3.4 Launch Services
```bash
# Start both n8n and Caddy containers
docker compose -f e2_docker-compose.yml up -d

# Verify service status
docker compose -f e2_docker-compose.yml ps
docker compose -f e2_docker-compose.yml logs n8n --tail=10

# Wait for services to be ready
sleep 30
curl -I https://n8n.aqmal.site
```

#### 3.5 Verify HTTPS Setup
```bash
# Test HTTPS redirection
curl -I http://n8n.aqmal.site  # Should redirect to HTTPS

# Test secure access
curl -I https://n8n.aqmal.site  # Should return 200 OK

# Check certificate details
openssl s_client -connect n8n.aqmal.site:443 -servername n8n.aqmal.site
```
